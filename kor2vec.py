...
def build_dataset(column='article', min_count=5, sampling_rate=0.0001):
    words = list(df[column].apply(lambda sentence: re.sub(r"[^ㄱ-힣a-zA-Z0-9]+", ' ', sentence).strip().split()))
    ...
    for word in word2index:
        word_tag_list = list()
        for tag in okt.pos(word, norm=True):
    ...
    word_corpus = sub_sampling(word_corpus, word_counter, word2index, sampling_rate)
...
def sub_sampling(word_corpus, word_counter, word2index, sampling_rate):
    total_words = sum([len(sentence) for sentence in word_corpus])
    probability_dict = dict()
    for word, count in word_counter:
        frequency = count / total_words
        probability = max(0, 1 - math.sqrt(sampling_rate / frequency))
...
def generating_wordpairs(indexed_corpus, tag_size, window_size=4):
    X = []
    Y = []
    for row in indexed_corpus:
        x, y = skipgrams(sequence=row, vocabulary_size=tag_size, window_size=window_size,
...
def construction_model(tag_size, embedding_dim):
    input_target = Input((3,))
    input_context = Input((3,))

    embedding_layer = Embedding(tag_size, embedding_dim, input_length=3)

    target_embedding = embedding_layer(input_target)
    target_embedding = Reshape((3, embedding_dim))(target_embedding)
    target_embedding_flatten = Reshape((embedding_dim * 3,))(target_embedding)
    target_dense = Dense(embedding_dim, activation='relu')(target_embedding_flatten)
    target_embedding_final = Reshape((embedding_dim, 1))(target_dense)

    context_embedding = embedding_layer(input_context)
    context_embedding = Reshape((3, embedding_dim))(context_embedding)
    context_embedding_flatten = Reshape((embedding_dim * 3,))(context_embedding)
    context_dense = Dense(embedding_dim, activation='relu')(context_embedding_flatten)
    context_embedding_final = Reshape((embedding_dim, 1))(context_dense)
...
def save_vectors(file_path, tag_size, embedding_dim, model, tag2index):
    f = open(file_path, 'w')
    f.write('{} {}\n'.format(tag_size-1, embedding_dim))
...
if __name__ == "__main__":
    ...
    model = construction_model(tag_size, embedding_dim=EMBEDDING_DIM)
    print("Model was constructed in\t{time} sec".format(time=time() - time_check))
    time_check = time()

    model = training_model(model, EPOCHS, BATCH_SIZE, indexed_corpus, tag_size)
    print("Traning was done in\t{time} sec".format(time=time() - time_check))
    time_check = time()

    save_path = save_vectors('vectors_on_batch.txt', tag_size, EMBEDDING_DIM, model, tag2index)
